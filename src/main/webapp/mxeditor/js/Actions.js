...
Actions.prototype.init = function()
{
	...
	this.addAction('getModel', function(){
		const cells = getCells(graph.getModel().cells[1]);
		console.log(cells);
		window.location = "/tutorial/marking?model=" + JSON.stringify(cells);
	});
...
const getCells = (cells) => {
	var array = [];
	if(cells.children){
		cells.children.forEach((children) => {
			let childrenCell = [];
			if(children.children){
				children.children.forEach((children) => {
					childrenCell.push(children.id);
				});
			}
			childrenCell = (childrenCell.length < 1) ? null : childrenCell;
			const source = (children.source) ? children.source.id : children.source;
			const target = (children.target) ? children.target.id : children.target;
			const style = defineStyle(children.style);
			array.push(new Data(children.id , children.value, children.parent.id, childrenCell, style, source, target));
			array.push(getCells(children));
		});
	}
	return array;
};
/**
 * 클라이언트사이드에서 다른 js파일에 있는 변수를 불러오지 못해서 여기다가 넣었다
 * 코드 중복 배드 코드
 */
const StyleEnum = {
	CLASS : 0,
...
const defineStyle = (style) => {
	let tmp;
	const length = Object.keys(StyleEnum.properties).length;
	for(let i = 0; i < length; i++){
		if(StyleEnum.search(style, i)) {
			tmp = StyleEnum.properties[i];
			break;
		}
	}
	return tmp;
...
class Data {
	constructor(id, value, parent, children, style, source, target){
		this.id = id;
		this.value = value;
		this.parent = parent;
